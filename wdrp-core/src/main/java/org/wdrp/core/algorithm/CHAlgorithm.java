...
public class CHAlgorithm extends AbstractRoutingAlgorithm<Arc> {
	static final Logger logger = Logger.getLogger(CHAlgorithm.class);
	...
	public void precompute() {
		if(!graph.isCH()) {
			...
			logger.info("Setting arc flags for all edges to true");
			graph.setArcFlagsForAllEdges(true); 
			
			//compute a random node ordering
			logger.info("Computing inital node ordering of contraction, based on edge difference");
			...
			logger.info("Starting contracting "+numOfContractions+ " nodes");
			...
			logger.info("Constructing upwards graph");
			constructUpwardsGraph(this.nodesHierachy);
			
			logger.info("total #shortcust added: "+this.numberOfShortcuts);
			graph.setCH(true);
		}
		else {
			logger.info("Graph already has shortcuts so skipping CH precomputation");
		}
	}
	...
	public void constructUpwardsGraph(TLongIntHashMap hierarchy) {
		...
		for (Tuple2<Long, Arc> arc : graph.adjacenyList) {
			...
			count++;
			if(count%100000==0) logger.debug("#arcs processed: " + count);
		}
	}
	...
	public TLongIntHashMap contractNodes(int numOfContractions, Queue<QEntry> order) {
		...
		for (int i = 0; i < numOfContractions; i++) {
			if(i>0 && i%100000==0) logger.debug("#nodes contracted: "+i);
